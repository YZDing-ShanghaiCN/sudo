...
root_path = "./20260507/wait_pose"
kind = "undistorted"
# kind = "origin"
# index_list = os.path.join(root_path, "diffx_list.txt")
# output_path = "/main-cpfs/yachi/test/middleburry/2014_dpt_ft_ts_bf16"
output_path_left = os.path.join(root_path, "depthmap", kind, "left")
output_path_right = os.path.join(root_path, "depthmap", kind, "right")
colormap_path_left = os.path.join(root_path, "colormap", kind, "left")
colormap_path_right = os.path.join(root_path, "colormap", kind, "right")

os.makedirs(output_path_left, exist_ok=True)
os.makedirs(output_path_right, exist_ok=True)
os.makedirs(colormap_path_left, exist_ok=True)
os.makedirs(colormap_path_right, exist_ok=True)
...
for i in range(20):
    ...
    left_image = Image.open(os.path.join(root_path, "rgb", "left_chest", kind, f"{i:06d}.jpg"))
    right_image = Image.open(os.path.join(root_path, "rgb", "right_chest", kind, f"{i:06d}.jpg"))
    ...
    Image.fromarray(metric_depth[0], mode="F").save(
        os.path.join(output_path_left, f"{i:06d}.tiff"), format="TIFF"
    )
    Image.fromarray(metric_depth[1], mode="F").save(
        os.path.join(output_path_right, f"{i:06d}.tiff"), format="TIFF"
    )
    save_depth_colormap(os.path.join(colormap_path_left, f"{i:06d}.png"), metric_depth[0])
    save_depth_colormap(os.path.join(colormap_path_right, f"{i:06d}.png"), metric_depth[1])
