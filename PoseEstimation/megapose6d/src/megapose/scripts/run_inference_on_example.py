...
def make_detections_visualization(
    ...
    rgb_det = rgb.copy()
    boxes = detections.bboxes.cpu().numpy().astype(np.int64)
    if "score" in detections.infos.columns:
        texts = [
            f"{row.label} {row.score:.2f}" for _, row in detections.infos.iterrows()
        ]
    else:
        texts = [f"{row.label}" for _, row in detections.infos.iterrows()]
    for box, text in zip(boxes, texts):
        x1, y1, x2, y2 = box
        cv2.rectangle(rgb_det, (x1, y1), (x2, y2), (255, 0, 0), 2)
        if text:
            text_y = max(0, y1 - 5)
            cv2.putText(
                rgb_det,
    ...
    cv2.imwrite(str(output_fn), rgb_det[..., ::-1])
...
def make_output_visualization(
    ...
    mesh_overlay = np.zeros_like(rgb).astype(np.float32)
    mesh_overlay[~mask] = rgb[~mask] * 0.6 + 255 * 0.4
    mesh_overlay[mask] = renderings.rgb[mask] * 0.8 + 255 * 0.2
    ...
    mesh_path = vis_dir / "mesh_overlay.png"
    contour_path = vis_dir / "contour_overlay.png"
    all_results_path = vis_dir / "all_results.png"
    all_results = np.concatenate([rgb, contour_overlay, mesh_overlay], axis=1)
    print(f"mesh_overlay shape: {mesh_overlay.shape}, save path: {mesh_path}")
    cv2.imwrite(str(mesh_path), mesh_overlay[..., ::-1])
    print(f"contour_overlay shape: {contour_overlay.shape}, save path: {contour_path}")
    cv2.imwrite(str(contour_path), contour_overlay[..., ::-1])
    print(f"all_results shape: {all_results.shape}, save path: {all_results_path}")
    cv2.imwrite(str(all_results_path), all_results[..., ::-1])
