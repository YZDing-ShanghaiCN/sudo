...
class Panda3dBatchRenderer:
    ...
    def _render_scene_sync(self, render_args: RenderArguments) -> RenderOutput:
        ...
        if is_valid:
            # Set copy_arrays=True so that the numpy arrays are contiguous with
            # non-negative strides and can be converted into torch.tensors.
            renderings = self._renderer.render_scene(
    ...
    def render(
        ...
        # Modern Python/PyTorch multiprocessing compatibility workaround:
        # execute Panda3D rendering synchronously in-process to avoid
        # Queue/Process pickle serialization failures.
        for n, scene_data_n in enumerate(scene_datas):
            ...
            renders = self._render_scene_sync(render_args)
    ...
    def _init_renderers(self, preload_cache: bool) -> None:
        ...
        preload_labels: Set[str] = set(object_labels) if preload_cache else set()
        # Modern Python/PyTorch multiprocessing compatibility workaround:
        # keep a single in-process renderer and avoid Process/Queue dispatch.
